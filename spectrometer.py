...
class SpectrometerApp(QMainWindow):
    ...
    def analyze_frame(self, frame_bgr):
        ...
        # 4. Calibration
        is_calibrated = self.calib_cb.isChecked() and self.px2.value() != self.px1.value()
        pixel_positions = np.arange(w)
        
        if is_calibrated:
            m = (self.wav2.value() - self.wav1.value()) / (self.px2.value() - self.px1.value())
            c_val = self.wav1.value() - m * self.px1.value()
            wavelengths = m * pixel_positions + c_val
        ...
        # Vibrant Graph Traces (Glow Effect) for Intensity
        self.ax_main.plot(x_vals, y_vals, color='#00d2ff', linewidth=6.0, alpha=0.2) # Outer glow
        self.ax_main.plot(x_vals, y_vals, color='#00d2ff', linewidth=3.0, alpha=0.5) # Inner glow
        self.ax_main.plot(x_vals, y_vals, color='#ffffff', linewidth=1.5) # Bright core
        
        # Fill area under the curve for extra vibrance
        self.ax_main.fill_between(x_vals, 0, y_vals, color='#00d2ff', alpha=0.15)
        
        if is_calibrated:
            # Highlight calibration points at their wavelength positions
            self.ax_main.axvline(x=self.wav1.value(), color='#ff4b4b', linestyle='--', alpha=0.5)
            self.ax_main.axvline(x=self.wav2.value(), color='#ff4b4b', linestyle='--', alpha=0.5)
            
            if len(peaks) > 0:
                self.ax_main.plot(x_vals[peaks], y_vals[peaks], 'ro', markersize=8, markeredgecolor='#ff4b4b', markeredgewidth=2, markerfacecolor='none')
                for p in peaks:
                    self.ax_main.text(x_vals[p], y_vals[p] + 12, f"{x_vals[p]:.1f} nm", color='#ff8888', fontsize=11, ha='center', weight='bold')
        else:
            if len(peaks) > 0:
                self.ax_main.plot(x_vals[peaks], y_vals[peaks], 'ro', markersize=8, markeredgecolor='#ff4b4b', markeredgewidth=2, markerfacecolor='none')
                for p in peaks:
                    self.ax_main.text(x_vals[p], y_vals[p] + 12, f"{x_vals[p]:.0f} px", color='#ff8888', fontsize=11, ha='center', weight='bold')
                    
        self.ax_main.set_ylim(0, 300)
        self.ax_main.set_xlim(x_vals[0], x_vals[-1])
            
        self.ax_main.set_xlabel(xlabel, color='white', fontsize=12, weight='bold')
        self.ax_main.set_ylabel(ylabel, color='white', fontsize=12, weight='bold')
        self.ax_main.grid(True, linestyle=":", alpha=0.4, color='#888888')
    ...
    def export_csv(self):
        ...
        if fname:
            pd.DataFrame({self.last_xlabel: self.last_x, self.last_ylabel: self.last_y}).to_csv(fname, index=False)
